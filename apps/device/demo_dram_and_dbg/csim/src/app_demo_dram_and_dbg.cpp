...
void app_demo_dram_and_dbg(
    ST_Queue<bool> &reset_app_demo_dram_and_dbg,
    ST_Queue<unsigned int> &dbg_peek_req, ST_Queue<unsigned int> &dbg_peek_resp,
    ...
    ST_Queue<APP_Data> &app_input_data, ST_Queue<APP_Data> &app_output_data,
  ...
  while (1) {
    bool dummy;
    if (reset || (reset = reset_app_demo_dram_and_dbg.read_nb(dummy))) {
      state = 0;
      is_read_op = 0;
      got_read_resp = false;
