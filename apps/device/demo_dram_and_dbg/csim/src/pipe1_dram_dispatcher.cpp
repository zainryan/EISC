...
void pipe1_dram_dispatcher(
    ...
    ST_Queue<bool> &dram_write_mux_apply_context,
  ...
  bool is_write_req_apply_from_device = false;
...
#pragma HLS pipeline
    ...
    if (!has_write_req) {
      if (unified_dram_dispatcher_write_context.read_nb(write_cmd)) {
        ...
        dram_write_mux_apply_context.read(is_write_req_apply_from_device);
      }
    }
    if (has_write_req) {
      if (unified_dram_write_req_data.read_nb(write_data)) {
        if (write_data.last && !is_write_req_apply_from_device) {
