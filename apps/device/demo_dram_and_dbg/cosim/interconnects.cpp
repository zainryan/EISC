...
void interconnects() {
  ...
  poke_handler(poke, dbg_peek_req, reqs_incoming, kbuf_addrs,
               dma_read_throttle_params, dma_write_throttle_params,
               drive_read_delay_cycle_cnts, drive_write_delay_cycle_cnts,
               drive_read_throttle_params, drive_write_throttle_params,
               app_file_infos_0, app_file_infos_1, read_mode_app_buf_addrs,
               write_mode_app_buf_addrs, app_free_buf, app_write_total_len,
               app_input_params, app_commit_write_buf, app_is_write_mode_0,
               app_is_write_mode_1, app_is_write_mode_2, reset_sigs);
  ...
  peek_handler(peek_req, peek_resp, peek_real_written_bytes_req,
               ...
               peek_virt_written_bytes_resp, dbg_peek_resp);
  ...
  pipe1_dram_dispatcher(
      ...
      cached_dramB_write_req_data, unified_dram_dispatcher_write_context,
  ...
  dram_C_D_to_app(cosim_dramC_read_req, cosim_dramC_read_resp,
                  cosim_dramC_write_req_data, cosim_dramC_write_req_apply,
                  cosim_dramD_read_req, cosim_dramD_read_resp,
                  cosim_dramD_write_req_data, cosim_dramD_write_req_apply,
                  app_dram_read_req, app_dram_read_resp,
                  app_dram_write_req_data, app_dram_write_req_apply);

  ST_Queue<bool> reset_app_demo_dram_and_dbg(4);
  reset_propaganda(
      reset_app_demo_dram_and_dbg, reset_sigs, reset_read_mode_dram_helper_app,
  ...
  ;
  app_demo_dram_and_dbg(reset_app_demo_dram_and_dbg, dbg_peek_req,
                        dbg_peek_resp, app_dram_read_req, app_dram_read_resp,
                        app_dram_write_req_data, app_dram_write_req_apply,
                        app_input_data, app_output_data, app_input_params);
}
