...
#pragma HLS pipeline
    ...
    if (has_read_req) {
      has_read_req = true;
      read_bank_id = (read_req.addr >> 12) & 1;
      unsigned long long read_bank_idx = read_req.addr >> 13;
      ...
      read_end_bank_id = (end_addr >> 12) & 1;
      unsigned long long read_end_bank_idx = end_addr >> 13;
    ...
    Dram_Write_Req_Apply ddr_write_apply[2];
    ...
    if (has_write_req) {
      has_write_req = true;
      write_bank_id = (write_apply.addr >> 12) & 1;
      unsigned long long write_bank_idx = write_apply.addr >> 13;
      unsigned long long dram_start_addr =
          (write_bank_idx << 12) + (write_apply.addr & 4095);
      ap_uint<DRAM_ADDR_BUS_WIDTH> end_addr =
          write_apply.addr + (write_apply.num << DATA_BUS_WIDTH_LOG2) - 1;
      write_end_bank_id = (end_addr >> 12) & 1;
      unsigned long long write_end_bank_idx = end_addr >> 13;
