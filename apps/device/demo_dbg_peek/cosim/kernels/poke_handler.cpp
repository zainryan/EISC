#ifndef POKE_HANDLER_CPP_
#define POKE_HANDLER_CPP_

#include <insider_kernel.h>

void poke_handler(
    ST_Queue<Poke_Info> &poke, ST_Queue<unsigned int> &dbg_peek_req,
    ST_Queue<Request> &reqs_incoming, ST_Queue<unsigned int> &kbuf_addrs,
    ...
    ST_Queue<bool> &app_is_write_mode_0, ST_Queue<bool> &app_is_write_mode_1,
    ST_Queue<bool> &app_is_write_mode_2, ST_Queue<bool> &reset_sigs) {
  unsigned int tag = 0;
  Request req;
...
#pragma HLS pipeline
    ...
    if (poke.read_nb(info)) {
      if (info.tag >= TAG(WRITE_CMD_TAG_OFFSET)) {
        ...
        state = 16;
      } else if (info.tag == TAG(APP_DBG_PEEK_TAG)) {
        state = 17;
      }
    }
