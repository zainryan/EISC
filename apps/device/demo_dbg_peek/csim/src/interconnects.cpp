...
int main() {
  ...
  ST_Queue<unsigned int> dbg_peek_req(4);
  ST_Queue<unsigned int> dbg_peek_resp(4);
  ST_Queue<Dram_Read_Req> app_dram_read_req(64);
  ST_Queue<Dram_Read_Resp> app_dram_read_resp(64);
  ST_Queue<Dram_Write_Req_Data> app_dram_write_req_data(64);
  ST_Queue<Dram_Write_Req_Apply> app_dram_write_req_apply(64);

  ST_Queue<bool> reset_app_demo_dbg_peek(4);

  ;
  std::thread t0(
      poke_handler, std::ref(poke), std::ref(dbg_peek_req),
      std::ref(reqs_incoming), std::ref(kbuf_addrs),
      std::ref(dma_read_throttle_params), std::ref(dma_write_throttle_params),
      std::ref(drive_read_delay_cycle_cnts),
      std::ref(drive_write_delay_cycle_cnts),
      std::ref(drive_read_throttle_params),
      std::ref(drive_write_throttle_params), std::ref(app_file_infos_0),
      std::ref(app_file_infos_1), std::ref(read_mode_app_buf_addrs),
  ...
  std::thread t1(
      ...
      std::ref(peek_virt_written_bytes_resp), std::ref(dbg_peek_resp));
  ...
  std::thread t33(
      ...
      std::ref(cached_dramB_write_req_data));
  std::thread t34(
      dram_C_D_to_app, std::ref(dramC_read_req), std::ref(dramC_read_resp),
      std::ref(dramC_write_req_data), std::ref(dramC_write_req_apply),
      std::ref(dramD_read_req), std::ref(dramD_read_resp),
      std::ref(dramD_write_req_data), std::ref(dramD_write_req_apply),
      std::ref(app_dram_read_req), std::ref(app_dram_read_resp),
      std::ref(app_dram_write_req_data), std::ref(app_dram_write_req_apply));
  std::thread t35(
      reset_propaganda, std::ref(reset_app_demo_dbg_peek), std::ref(reset_sigs),
      std::ref(reset_read_mode_dram_helper_app),
      std::ref(reset_write_mode_dram_helper_app),
      std::ref(reset_read_mode_pcie_helper_app),
      std::ref(reset_write_mode_pcie_helper_app),
      ...
      std::ref(reset_app_input_data_merger),
      std::ref(reset_write_mode_pre_merged_app_input_data_forwarder));
  std::thread t36(app_demo_dbg_peek, std::ref(reset_app_demo_dbg_peek),
                  std::ref(dbg_peek_req), std::ref(dbg_peek_resp),
                  std::ref(app_dram_read_req), std::ref(app_dram_read_resp),
                  std::ref(app_dram_write_req_data),
                  std::ref(app_dram_write_req_apply), std::ref(app_input_data),
