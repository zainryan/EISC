...
int vsync(int fd);
unsigned int dbg_peek(unsigned int req);
...
unsigned int dbg_peek(unsigned int req) {
  pthread_mutex_lock(&mutex);
  send_control_msg(TAG(APP_DBG_PEEK_TAG), req);
  unsigned int ret = receive_control_msg(TAG(APP_DBG_PEEK_TAG));
  pthread_mutex_unlock(&mutex);
  return ret;
}
...
ST_Queue<unsigned int> dbg_peek_req(1);
ST_Queue<unsigned int> dbg_peek_resp(1);
ST_Queue<Dram_Read_Req> app_dram_read_req(1);
ST_Queue<Dram_Read_Resp> app_dram_read_resp(1);
ST_Queue<Dram_Write_Req_Data> app_dram_write_req_data(1);
ST_Queue<Dram_Write_Req_Apply> app_dram_write_req_apply(1);
#endif
